		...
		if (tree !== null) {
			...
			$("#_tree").fancytree({
				...
				renderColumns : function(event, data) {
					...
					$tdList.eq(11).text(node.data.amount);
					// 					$tdList.eq(11).text(node.data.color);
					$tdList.eq(12).text(node.data.applyColor);
					// 					$tdList.eq(12).html("<input type='checkbox' class='isBox c1" + node.key + "' name='c1' value='true' disabled='disabled'>");
					// 					$tdList.eq(13).html("<input type='checkbox' class='isBox s1" + node.key + "' name='s1'" + (node.data.isColorSet == true ? "checked='checked'" : '') + " value='true' disabled='disabled'>");
					$tdList.eq(13).text(node.data.erpCode);
				},
				init : function() {
					$("input[name=c1]").checks();
					// 					$("input[name=s1]").checks();
					mask.close();
					$("#loading_layer").hide();
					_selector("color");
