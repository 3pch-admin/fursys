...
<%@page import="platform.mbom.service.MBOMHelper"%>
...
MBOM mbom = MBOMHelper.manager.getMBOM(part);
if (mbom != null) {
	MBOMHeader header = mbom.getHeader();
	WTPart headerPart = header.getPart();
%>
<table class="search-table mbom-search close">
	<colgroup>
		<col width="180">
		<col width="530">
		<col width="180">
	...
	<tr>
		<th>조회</th>
		<td>
			<input type="text" name="msearch" class="AXInput w70p">
		</td>
		<th>색상(CAD 매개변수)</th>
		<td><%=IBAUtils.getStringValue(headerPart, "COLOR")%></td>
		<th>파생색상</th>
		<td>
			<select name="mcolor" id="mcolor" class="AXSelect w200px">
...
<script type="text/javascript">
	...
	$(function() {
		...
		$("input[name=msearch]").on("keyup", function(e) {
			var match = $(this).val();
	...
	var color = $("select[name=mcolor] option:selected").val();
	<%if (header != null) {%>
