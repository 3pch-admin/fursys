...
%>
<table class="search-table pbom-search close">
	<colgroup>
		<col width="180">
		<col width="530">
		<col width="180">
	...
	<tr>
		<th>조회</th>
		<td>
			<input type="text" name="psearch" class="AXInput w70p">
		</td>
		<th>색상(CAD 매개변수)</th>
		<td><%=IBAUtils.getStringValue(headerPart, "COLOR")%></td>
		<th>파생색상</th>
		<td>
			<select name="pcolor" id="pcolor" class="AXSelect w200px">
...
<br class="pbr close">
<table class="create-table pbom close" id="_ptree">
	<colgroup>
		<col width="40">
		<col width="400">
		<col width="180">
...
<script type="text/javascript">
	...
	$(function() {
		...
		$("input[name=psearch]").on("keyup", function(e) {
			var match = $(this).val();
			var n, treeRight = $.ui.fancytree.getTree("#_ptree");
			var opts = {};
	...
	function loadPTree(oid, color) {
		var tree = $.ui.fancytree.getTree("#_ptree");
		if (tree !== null) {
			$("#pcolor").unbindSelect();
			tree.clear();
			preload(tree, oid, color);
		} else {
		$("#_ptree").fancytree({
	...
	function preload(tree, oid, color) {
...
%>
<table class="create-table pbom-search close">
	<colgroup>
		<col width="40">
		<col width="400">
		<col width="180">
